...
try{
 const page=await browser.newPage({viewport:{width:1200,height:700}});await page.goto('http://localhost:8770');await page.waitForFunction(()=>window.__turf?.state.assetReady);
 const samples=await page.evaluate(async()=>{
  const {RacingScene}=await import('/src/scene.js');const {createRoster}=await import('/src/race.js');
 ...
 await page.evaluate(()=>{gaitCheck.update([{id:1,distance:300,lane:7,velocity:17,gaitPhase:.84}],0,0,true);for(const h of gaitCheck.horses)h.group.visible=h.id===1;gaitCheck.render();});await page.screenshot({path:'artifacts/gait-suspension.png'});
 ...
 const audio=await page.evaluate(async()=>{const {RaceAudio}=await import('/src/audio.js');const a=new RaceAudio();await a.toggle();a.update(.016,true,.2,1,[{velocity:17}]);const result={duration:a.buffer.duration,channels:a.buffer.numberOfChannels,loop:a.source.loop,rate:a.source.playbackRate.value};a.silence();result.stopped=a.source===null;await a.ctx.close();return result;});
